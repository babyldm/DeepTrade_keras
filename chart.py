...
def extract_feature(raw_data, selector, window=30, with_label=True, flatten=True):
    ...
    closes = raw_data.close.values.astype(float)
    opens = raw_data.open.values.astype(float)
    highs =  raw_data.high.values.astype(float)
    lows =  raw_data.low.values.astype(float)
    volumes =  raw_data.volume.values.astype(float)
    if with_label:
